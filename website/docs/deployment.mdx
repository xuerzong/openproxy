---
title: 部署
description: 通过 Docker Compose 或手动方式部署 OpenProxy
---
...
该脚本会基于 `docker/.env.example` 自动生成 `docker/.env`，并补齐 `BETTER_AUTH_SECRET`、`CRON_SECRET`、`RSA_PRIVATE_KEY`、`RSA_PUBLIC_KEY` 等必要值。
...
定时任务配置与调用方式见 [定时任务](/scheduled-jobs)。

## 生产环境检查清单

- [ ] 所有公开域名均启用 HTTPS 并使用有效证书
- [ ] `BETTER_AUTH_URL` 已设置为对外前端域名加 `/api`，`APP_DOMAIN` 已正确配置为部署使用的主域名，以匹配内置 trusted origins 和跨子域 cookie
- [ ] 开启 PostgreSQL 自动备份（如定时 pg_dump 或托管数据库快照）
- [ ] 已配置 `cleanupOrder` 和 `archiveMonthlyUsage` 的定时触发，并确保请求携带正确的 `CRON_SECRET`
