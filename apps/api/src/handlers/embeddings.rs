...
use crate::{
    ...
    shared::{
        AppState,
        chat_common::ChatProxyHandler,
        proxy::PreparedUpstreamRequest,
        proxy_flow::{
            ProxyRequestHandler, ProxyResponseContext, ResponseFuture, execute_proxy_request,
        },
    },
...
pub async fn embeddings_handler(
    ...
    execute_proxy_request::<EmbeddingsRequestHandler>(state, user, req).await
}

struct EmbeddingsRequestHandler {
    inner: ChatProxyHandler,
}

impl ProxyRequestHandler for EmbeddingsRequestHandler {
