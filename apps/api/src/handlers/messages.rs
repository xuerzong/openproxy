...
pub async fn messages_handler(
    ...
    execute_proxy_request::<MessagesRequestHandler>(state, user, req).await
}

struct MessagesRequestHandler {
    inner: ChatProxyHandler,
}

impl ProxyRequestHandler for MessagesRequestHandler {
    fn try_new(req_path: String, body_json: serde_json::Value) -> Result<Self, Response> {
        Ok(Self {
            inner: ChatProxyHandler::new(req_path, body_json, UsageStyle::Anthropic),
        })
    }
